		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="店铺名称" prop="store_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['merchant_users_id'] && $check_field('set','store_name')) || (!form_sub['merchant_users_id'] && $check_field('add','store_name'))"></el-input>
					<div v-else-if="$check_field('get','store_name')">{{form_sub['store_name']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','sellers_name') || $check_field('add','sellers_name') || $check_field('set','sellers_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="卖家姓名" prop="sellers_name">
												<el-input id="sellers_name" v-model="form_sub['sellers_name']" placeholder="请输入卖家姓名"
							  v-if="user_group === '管理员' || (form_sub['merchant_users_id'] && $check_field('set','sellers_name')) || (!form_sub['merchant_users_id'] && $check_field('add','sellers_name'))"></el-input>
					<div v-else-if="$check_field('get','sellers_name')">{{form_sub['sellers_name']}}</div>
											</el-form-item>
			</el-col>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["merchant_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?merchant_users_id=" + form_sub["merchant_users_id"],form_sub,(res)=>{
