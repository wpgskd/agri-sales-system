<template>
	<div class="diy_details page_commodity_center" id="commodity_center_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_commodity_center'>
							<div_commodity_center :obj="obj"></div_commodity_center>
...
<script>
	import div_commodity_center from "@/components/diy/div_commodity_center.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/commodity_center/set?commodity_center_id=' + obj["commodity_center_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.commodity_center_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "commodity_center",
					source_field: "commodity_center_id",
					source_id: obj["commodity_center_id"],
					user_id
				};
