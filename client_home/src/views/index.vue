		            ...
		            <div class="col-12">
		                <div class="sw_no">
							...
							<div class="card_notice">
								<div class="notice_title">公告信息 <router-link to="notice/list">更多</router-link></div>
								<!-- 公告组件 -->
		    ...
		    <div class="container">
		        <div class="row">
		            <div class="col">
						...
						<!-- 推荐商品中心模块(开始) -->
		<div class="warp" v-if="$check_action('/commodity_center/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/commodity_center/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_commodity_center = !show_list_commodity_center">
		                        <div class="switch_box">
		                            <span v-if="show_list_commodity_center"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 商品中心推荐列表组件 -->
		                <list_commodity_center v-if="show_list_commodity_center" :list="list_commodity_center" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_commodity_center.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_commodity_center" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_commodity_center" :key="i" @click="to_details('commodity_center',o,'commodity_center_id')">
		                            <td v-for="(oj,n) in fields_commodity_center">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
								show_list_commodity_center: true,
				list_commodity_center: [],
				vm_arr_commodity_center: [
					"commodity_center_id",
							"store_name",
							"seller_customers",
							"item_number",
		...
		methods: {
			...
			toToggle(){
				this.isAdmin = !this.isAdmin;
							...
							// 获取商品中心列表
			get_commodity_center() {
				let url = "~/api/commodity_center/get_list?";
										url = "~/api/commodity_center/get_hits_list?";
						this.$get(url, {
					...
					if (json.result) {
						this.list_commodity_center = json.result.list;
					}
				})
			},
		...
		computed: {
						    list_table_sales_data() {
		        var list = this.list_commodity_center;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_commodity_center.map((o) => {
		                // 第二个中括号是对象的属性
