<template>
	<div class="diy_details_box diy_div_commodity_center">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
											<button class="btn btn-primary details_btn" v-if="$check_action('/order_management/edit','add')" @click="to_form('/order_management/edit')" > 购买 </button>
											<button class="btn btn-primary details_btn" v-if="$check_action('/commodity_center/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/commodity_center/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "commodity_center_id",
					source_id: this.obj["commodity_center_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/commodity_center/set?commodity_center_id=' + _this.obj["commodity_center_id"], {
								praise_len
							}, (res) => {
								if(res.result){
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
																							query.title = this.obj.trade_name
																								query.img = this.obj.product_cover
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var commodity_center_id = this.obj.commodity_center_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "commodity_center",
          source_field: "commodity_center_id",
          source_id: commodity_center_id
        }, (res) => {
